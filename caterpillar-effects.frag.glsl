...
vec3 sphereShift(int i, float n) {
  ...
  return vec3(cos(t) * 2., sin(t * 2.), 0); // (sin(t * 3.) + 1.) / 2.);
}
...
float sdf(vec3 uv) {
  ...
  if (uv.x < 0. && uv.y < 0.) {
    float s = 7.;
    uv *= s;
    uv = floor(uv) + fract(uv) * sqrt(fract(uv));
    uv /= s;
  }

  if (uv.x >= 0. && uv.y >= 0.) {
    float f = 5.;
    uv.z += (cos(uv.x * f) + sin(uv.y * f)) / f;
  }

  for (int i; float(i) < n; i++) {
    vec3 center = sphereCenter + sphereShift(i, n);
    vec3 center_ = sphereCenter + sphereShift(i - 1, n);
    float sdf_ = sdSphere(uv, center, r);

    //sdf = min(sdf, sdf_);
    sdf = opSmoothUnion(sdf, sdf_, length(center - center_) - r + 0.1);
  }
...
float sdfLight(vec3 uv, vec3 light, float eps) {
  ...
  if (uv.x < 0.) angle = pow((angle + 1.) / 2., 3.) * 2. - 1.;
  return angle;
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
  ...
  vec3 uvNormal = uv;
  vec3 light = sphereCenter - vec3(0, 0, 1.) + sphereShift(int(sphereCount) + 2, sphereCount);
